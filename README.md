...
* Restrict access to an object based on the user's IP address: Lets you define a set of IP ranges that object are allowed to be accessed from. Users outside these ranges receive an "Access denied" message. Thubnails are excluded from this restriction (that is, users outside the allowed IP ranges can view a restricted object's thumbnail).
